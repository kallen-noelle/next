...
export default function ArticleNav({ prev, next }: { prev: ArticleNavType | null; next: ArticleNavType | null }) {
  return (
    <div className="grid grid-cols-2 gap-4 mt-10">
      {prev ? (
        <Link href={`/article/${prev.id}`} className="glass-card p-4 group hover:border-indigo-400 transition-colors">
      ...
      {next ? (
        <Link href={`/article/${next.id}`} className="glass-card p-4 text-right group hover:border-indigo-400 transition-colors">
