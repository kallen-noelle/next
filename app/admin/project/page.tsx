...
export default function AdminProjectPage() {
  ...
  const handlePublish = async (id: number) => { await api.put(`/project/${id}/publish`); refresh(keyword || undefined, pageNum, pageSize); };
  const handleUnpublish = async (id: number) => { await api.put(`/project/${id}/unpublish`); refresh(keyword || undefined, pageNum, pageSize); };
          ...
          {items.map((p) => (
            <div key={p.id} className="glass-card px-4 py-3 flex items-center gap-4 group">
              ...
              <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                <Link href={`/admin/project/${p.id}/edit`} title="Edit" className="p-1 text-indigo-400 hover:text-indigo-600 transition-colors">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                </Link>
                <button onClick={() => p.isPublished ? handleUnpublish(p.id!) : handlePublish(p.id!)} title={p.isPublished ? "Unpublish" : "Publish"} className="p-1 text-emerald-400 hover:text-emerald-600 transition-colors">
                  {p.isPublished ? (
                ...
                <button onClick={() => handleDelete(p.id!)} title="Delete" className="p-1 text-red-400 hover:text-red-600 transition-colors">
