...
import { getList, remove } from "@/lib/api/chatter";
...
export default function AdminChatterPage() {
  ...
  const [items, setItems] = useState<Chatter[]>([]);
  ...
  const handleDelete = async (id: number) => {
    const ok = await confirm("删除此说说？"); if (!ok) return;
  ...
  return (
    <div className="flex flex-col flex-1 min-h-0">
      <h1 className="text-2xl font-black text-slate-900 dark:text-white mb-6">Chatters</h1>
      ...
      <div className="flex gap-3 mb-6">
        ...
        <Link href="/admin/chatter/new" className="px-4 py-2 bg-indigo-500 hover:bg-indigo-600 text-white text-sm font-bold rounded-xl transition-colors inline-flex items-center">
          New Chatter
        </Link>
      </div>
      ...
      <div className="flex-1 overflow-auto">
        <div className="grid gap-3">
          {items.map((c) => (
            <div key={c.id} className="glass-card !rounded-xl px-5 py-4 flex items-center gap-4 group">
              <div className="flex-1 min-w-0">
                <p className="text-sm text-slate-700 dark:text-slate-300 line-clamp-2">{c.content}</p>
                <p className="text-xs text-slate-400 mt-1">
                  {c.mood && <span className="mr-2">心情: {c.mood}</span>}
                  {c.createTime && new Date(c.createTime).toLocaleDateString("zh-CN")}
                </p>
              </div>
              <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                <Tooltip text="Edit">
                  <Link href={`/admin/chatter/${c.id}/edit`} className="p-1.5 text-indigo-400 hover:text-indigo-600 transition-colors block">
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                  </Link>
                </Tooltip>
                <Tooltip text="Delete">
                  <button onClick={() => handleDelete(c.id!)} className="p-1.5 text-red-400 hover:text-red-600 transition-colors">
          ...
          {items.length === 0 && <p className="text-center text-sm text-slate-400 py-10">No chatters yet.</p>}
        </div>
      </div>
