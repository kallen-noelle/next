...
export default function AdminArticlePage() {
  ...
  const handlePublish = async (id: number) => { await api.put(`/article/${id}/publish`); refresh(keyword || undefined, pageNum, pageSize); };
  const handleUnpublish = async (id: number) => { await api.put(`/article/${id}/unpublish`); refresh(keyword || undefined, pageNum, pageSize); };
  const handlePin = async (id: number) => { await api.put(`/article/${id}/pin`); refresh(keyword || undefined, pageNum, pageSize); };
          ...
          {items.map((a) => (
            <div key={a.id} className="glass-card px-4 py-3 flex items-center gap-4 group">
              ...
              <div className="flex items-center gap-0.5 opacity-0 group-hover:opacity-100 transition-opacity">
                <Link href={`/admin/article/${a.id}/edit`} title="Edit" className="p-1 text-indigo-400 hover:text-indigo-600 transition-colors">
                  <svg className="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z" /></svg>
                </Link>
                <button onClick={() => a.isPublished ? handleUnpublish(a.id!) : handlePublish(a.id!)} title={a.isPublished ? "Unpublish" : "Publish"} className="p-1 text-emerald-400 hover:text-emerald-600 transition-colors">
                  {a.isPublished ? (
                ...
                <button onClick={() => handlePin(a.id!)} title={a.isPinned ? "Unpin" : "Pin"} className="p-1 text-amber-400 hover:text-amber-600 transition-colors">
                ...
                <button onClick={() => handleDelete(a.id!)} title="Delete" className="p-1 text-red-400 hover:text-red-600 transition-colors">
